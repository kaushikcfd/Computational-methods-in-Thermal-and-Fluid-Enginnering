function [ust1, ust2, ust3, u1, u2, u3, u4, u5, t] = ftcs(dx,dt)
t = 0;
u1 = zeros(10000,1);
u2 = zeros(10000,1);
u3 = zeros(10000,1);
...
while(err>1)
    ...
    for i = 2:size(x,2)-1
        for j = 2:size(y,2)-1
            for k = 2:size(z,2)-1
                u(i,j,k) = u_prev(i,j,k) + d*(u_prev(i+1,j,k) + u_prev(i-1,j,k) + u_prev(i,j+1,k) + u_prev(i,j-1,k) + u_prev(i,j,k+1) + u_prev(i,j,k-1) - 6*u_prev(i,j,k));
            end
        end
    end
